    ...
    bufferCanvas.height = renderer.clientHeight * 1;
       ...
       let scale = Math.min(
        ...
        bufferCanvas.height = Math.round(scale * canvas.height);
          ...
          const preloadTextures = ['montane.jpg', 'steppe.jpg']; //fixed
...
function createCell(x, y, cellSize, data) {
    ...
    cardboard.classList.add(`Big`);
    cardboard.style.backgroundImage = `url("${data}")`;
...
function createTextInput() {
    ...
    header.id = 'header';
    header.textContent = 'Past your decklist in Moxfield format:'

    const textArea = document.createElement('textarea');
    textArea.id = 'decklist';
    textArea.placeholder = 'paste here, then click confirm, then walk (WASD)... Firefox not supported xD'
    ...
    textCont.appendChild(textArea);
    textCont.appendChild(confirm);
...
async function getCardImageUrls(decklist) {
    ...
    for (const line of lines) {
        ...
        if (match) cardNames.push(match[1]);
    }
    ...
    for (const name of cardNames) {
        try {
            ...
            console.warn(`Could not do this shit.. ${name}:`, err);
        }
    }
